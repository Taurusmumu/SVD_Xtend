...
def flip_img_generate(root_dir, out_dir, slide_name, patch_name, target_layers, start_layer, frame_num=16):
    """
    ...
    try:
        ...
        motion_scores2 = calculate_motion2(output_3D)
...
if __name__ == '__main__':
    ...
    with open(anno_path, "r") as rf:
        ...
        for line in tqdm(rf.readlines(), desc="Processing data"):
            ...
            min_sum = 100
            min_index = 0
            for i in range(len(target_layers) - 15):
                section = blur_scores[i:i + 16]
                if np.sum(section) < min_sum:
                    min_sum = np.sum(section)
                    min_index = i

            result = flip_img_generate(root_dir, out_dir, slide_name, patch_name, target_layers, min_index)
