...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        gt_images: List[PIL.Image.Image] = None,
        ...
        if cond is not None and gt_images is not None:
            self.transform = transforms.Compose([
                # transforms.Resize(height, interpolation=transforms.InterpolationMode.BICUBIC),
                # transforms.CenterCrop((height, width)),
                transforms.Lambda(lambda img: img.convert("RGB")),  # _convert_to_rgb
                transforms.ToTensor(),
                transforms.Normalize([0.5], [0.5]),
            ])
            gt_images_tensor = torch.empty((1, len(gt_images), 3, height, width))
            for i in range(len(gt_images)):
                gt_images_tensor[:, i] = self.transform(gt_images[i])
            gt_images_tensor_ = rearrange(gt_images_tensor, "b f c h w -> (b f) c h w").to(device)
            gt_image_latents = self.vae.encode(gt_images_tensor_).latent_dist.sample()
            gt_image_latents = rearrange(gt_image_latents, "(b f) c h w -> b f c h w", f=num_frames)
            gt_image_latents = gt_image_latents * self.vae.config.scaling_factor

            mask = cond.unsqueeze(0).unsqueeze(2).unsqueeze(3).unsqueeze(4).float()
            mask = mask.expand(-1, -1, gt_image_latents.shape[2], gt_image_latents.shape[3], gt_image_latents.shape[4])
        ...
        num_steps_half = (num_frames + 1) // 2
        second_half = torch.linspace(min_guidance_scale, max_guidance_scale, num_steps_half)
        # Create the first half by reversing the second half.
        first_half = torch.flip(second_half, dims=[0])
        #when the total number of frames is odd.
        guidance_scale_v_shape = torch.cat([first_half, second_half[1:]]).unsqueeze(0)

        # print(f"--- New 'V-Shape' Ramp for {num_frames} frames ---")
        # print(guidance_scale_v_shape)
        # print(f"\nShape: {guidance_scale_v_shape.shape}")
        guidance_scale = guidance_scale_v_shape
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if gt_images is not None and mask is not None:
                    latents = latents * (1 - mask) + \
                              (gt_image_latents + self.scheduler.sigmas[self.scheduler.step_index]
                               * noise_pred) * mask
