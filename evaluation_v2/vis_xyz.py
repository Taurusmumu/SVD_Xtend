...
def gen_xyz_gif(paths, out_dir, base_name, fps=5):
    ...
    for path in paths:
        ...
        frames.append(frame)
    export_to_gif(frames, os.path.join(out_dir, f"{base_name}_xy.gif"), duration=150)
    ...
    def make_gif(frames, path, fps=5):
        ...
        imageio.mimsave(path, frames_uint8, duration=150)
...
def main(config_path):
    ...
    for info in slide_patch_pairs:
        ...
        gt_paths = [os.path.join(gt_video_path, f) for f in sorted(os.listdir(gt_video_path))]
        out_dir = os.path.join(config['output_path'], config['gif_folder'], slide, patch_name)
        os.makedirs(out_dir, exist_ok=True)
        gen_xyz_gif(gt_paths, out_dir, "gt", fps=5)

        pred_paths = [os.path.join(pred_video_path, f) for f in sorted(os.listdir(pred_video_path))]
        gen_xyz_gif(pred_paths, out_dir, "pred", fps=5)
