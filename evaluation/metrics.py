...
class MetricsCalculator:
    ...
    def _calculate_ssim_ver(self, gt_np, pred_np):
        ...
        gt_scores = []
        for i in range(num_frames):
            score = ssim(mid_gt, gt_np[i], multichannel=True, channel_axis=2, data_range=1.0)
            gt_scores.append(score)

        pred_scores = []
        for i in range(num_frames):
            score = ssim(mid_pred, pred_np[i], multichannel=True, channel_axis=2, data_range=1.0)
            pred_scores.append(score)

        compare_scores = np.abs(np.array(gt_scores) - np.array(pred_scores))
    ...
    def evaluate(self, original_video, recon_video):

        results = {}

        # Prepare tensors for different libraries
        # For scikit-image (ssim): convert to numpy, HWC, range [0, 1]
        gt_np = np.array(original_video, dtype=np.float32) / 255.0
        pred_np = np.array(recon_video, dtype=np.float32) / 255.0
        ...
        gt_tensor = torch.empty((len(original_video), 3, original_video[0].size[0], original_video[0].size[1]), device=self.device)
        pred_tensor = torch.empty((len(recon_video), 3, recon_video[0].size[0], recon_video[0].size[1]), device=self.device)
        for i in range(len(original_video)):
            gt_tensor[i] = transform_pipeline(original_video[i]).to(self.device)
            pred_tensor[i] = transform_pipeline(recon_video[i]).to(self.device)
